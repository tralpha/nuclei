...
def random_poisson_noise(image, u=0.5):
    if random.random() < u:
        peak = np.random.choice([4.0, 8.0, 32.0, 64.0, 128.0])
        image = np.random.poisson(image.astype(float) / 255.0 *
                                        peak) / peak * 255
        image = np.clip(image, 0, 255).astype(np.uint8)
    return image

def random_color_pertubation(image, u=0.5):
    # o_image = image.copy()
    if random.random() < u:
        ...
        red = image[:,:,2].copy()
        if color == 1:
            multiplier = np.random.choice([1.1, 1.2, 1.3, 1.4, 1.5, 1.6])
            image[:,:,0] = blue*multiplier
            # debug o_image = cv2.cvtColor(o_image, cv2.COLOR_BGR2RGB); 
            # multiplier = np.random.choice(np.arange(1,2.1,0.1)); multiplier;
            # image[:,:,0] = blue*multiplier; 
            # d_image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB); 
            # plt.imshow(d_image); plt.show()
        elif color == 2:
        ...
        #     image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        # elif color == 1:
        #     image[:,:,0] = blue*np.random.choice([1.1, 1.2, 1.3, 1.4, 1.5])
        # elif color == 2:
        #     image[:,:,2] = (red*1.5).astype(np.uint8)
        #     image[:,:,1] = (green*1.5).astype(np.uint8)
        #     image[:,:,0] = (blue*1.25).astype(np.uint8)
        # image = np.clip(image, 0, 255).astype(np.uint8)
        # from IPython.core.debugger import set_trace; set_trace()
    return image


def random_gaussian_blur(image, u=0.5):
    o_image = image.copy()
    if random.random() < u:
        blur = iaa.GaussianBlur(sigma=(1.0, 3.0))
