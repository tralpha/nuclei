...
def revert(net, images):
    ...
    for b in range(batch_size):
        ...
        net.detections = torch_clip_proposals(net.detections, index, width,
                                              height)
...
def run_submit():
    ...
    if initial_checkpoint is not None:
        ...
        net.load_state_dict(
            torch.load(
                initial_checkpoint, map_location=lambda storage, loc: storage))
    ...
    for i, (inputs, images, indices) in enumerate(test_loader, 0):
        ...
        assert (
            batch_size == 1
        )  #note current version support batch_size==1 for variable size input
        ...
        for b in range(batch_size):
            ...
            cv2.imwrite(out_dir + '/submit/psds/%s/%s.png' % (name, name),
                        image)
            cv2.imwrite(out_dir + '/submit/psds/%s/%s.mask.png' % (name, name),
                        color_overlay)
            cv2.imwrite(out_dir + '/submit/psds/%s/%s.contour.png' %
                        (name, name), contour_overlay)
...
def run_npy_to_sumbit_csv():
    ...
    image_dir = ('/Users/ralphtigoumo/Documents/kaggle/nuclei/'
                 '20180306/data/image/stage1_test/images')
    ...
    for npy_file in npy_files:
        ...
        contour_overlay = multi_mask_to_contour_overlay(multi_mask, image,
                                                        [0, 255, 0])
        all = np.hstack(
            (image, contour_overlay, color1_overlay)).astype(np.uint8)
        image_show('all', all)
        cv2.waitKey(1)
    ...
    df = pd.DataFrame({
        'ImageId': cvs_ImageId,
        'EncodedPixels': cvs_EncodedPixels
    })
