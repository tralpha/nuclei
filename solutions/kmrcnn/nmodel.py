...
class UpSamplingTf(KE.Layer):
    def __init__(self, scale=2, **kwargs):
        self.scale = scale
        super(UpSamplingTf, self).__init__()

    def build(self, input_shape):
        self.shape = input_shape
        super(UpSamplingTf, self).build(input_shape)

    def call(self, inputs, **kwargs):
        # new_size = self.compute_output_shape(self.shape)
        new_size = K.shape(inputs[1])[1:3]
        # set_trace()
    ...
    def compute_output_shape(self, input_shape):
        # set_trace()
        # output_shape = list(input_shape)[1][:3]+[256]
        output_shape = (None, None, None, 256)
...
def resnet_graph(input_image, architecture, stage5=False, use_resnext=False):
    ...
    x = conv_block(
        ...
        cardinality=cardinality)
    x = identity_block(
        x, 3, [64, 64, 256], stage=2, block='b', cardinality=cardinality)
    C2 = x = identity_block(
        x, 3, [64, 64, 256], stage=2, block='c', cardinality=cardinality)
    # Stage 3
    x = conv_block(
        x, 3, [128, 128, 512], stage=3, block='a', cardinality=cardinality)
    x = identity_block(
        x, 3, [128, 128, 512], stage=3, block='b', cardinality=cardinality)
    x = identity_block(
        x, 3, [128, 128, 512], stage=3, block='c', cardinality=cardinality)
    C3 = x = identity_block(
        x, 3, [128, 128, 512], stage=3, block='d', cardinality=cardinality)
    # Stage 4
    x = conv_block(
        x, 3, [256, 256, 1024], stage=4, block='a', cardinality=cardinality)
...
class ProposalLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        anchors = inputs[2][0]
        # set_trace()
        img_shape = K.shape(inputs[3])

        # Improve performance by trimming to top anchors by score
        # and doing the rest on the smaller subset.
        # pre_nms_limit = min(6000, self.anchors.shape[0])
        pre_nms_limit = K.minimum(6000, K.shape(anchors)[0])
        ix = tf.nn.top_k(
        ...
        window = K.cast(K.stack([0, 0, height, width]), dtype='float32')
        boxes = utils.batch_slice(
            boxes,
        ...
        normalized_boxes = boxes / K.cast(
            K.stack([[height, width, height, width]]), dtype='float32')
...
class PyramidROIAlign(KE.Layer):
    ...
    def call(self, inputs):
        ...
        # Obtain image shape as an input tensor to this layer
        im_shape = K.shape(inputs[1])
...
def refine_detections(rois, probs, deltas, window, image_shape, config):
    ...
    refined_rois = utils.apply_box_deltas(rois, deltas_specific *
                                          config.BBOX_STD_DEV)
...
def fpn_classifier_graph(rois, feature_maps, in_image, pool_size, num_classes):
    ...
    x = PyramidROIAlign(
        [pool_size, pool_size], (1024, 1024, 3),
        name="roi_align_classifier")([rois, in_image] + feature_maps)
    ...
    x = KL.Activation('relu')(x)
    x = KL.TimeDistributed(
        KL.Conv2D(1024, (1, 1)), name="mrcnn_class_conv2")(x)
...
def build_fpn_mask_graph(rois, feature_maps, in_image, pool_size, num_classes):
    ...
    # x = PyramidROIAlign(
    #     [pool_size, pool_size], image_shape,
    #     name="roi_align_mask")([rois] + feature_maps)

    x = PyramidROIAlign(
        [pool_size, pool_size], (1024, 1024, 3),
        name="roi_align_mask")([rois, in_image] + feature_maps)
    # set_trace()
...
def data_generator(dataset,
    ...
    while True:
        try:
            ...
            # Load anchors in the generator
            anchors = utils.generate_pyramid_anchors(
                image.shape, config.RPN_ANCHOR_SCALES,
            ...
            if b == 0:
                ...
                batch_rpn_anchors = np.zeros(
                    [batch_size, anchors.shape[0], anchors.shape[1]],
            ...
            if b >= batch_size:
                inputs = [
                    batch_images, batch_image_meta, batch_rpn_anchors,
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        # C2, C3, C4, C5 = resnet101_graph(input_image)
        # Top-down Layers
        # TODO: add assert to varify feature map sizes match what's in config
        P5 = KL.Conv2D(256, (1, 1), name='fpn_c5p5')(C5)
        # P4_size = K.shape(C4)

        P5_upsampled = UpSamplingTf(name="P5_upsampled")([P5, C4])

        # P5_upsampled = KL.Lambda(
        #     lambda x: upsampling_graph(*x),
        #     name="fpn_p5upsampled")([P5, K.shape(C4)[1:3]])

        # P5_upsampled = KL.Lambda(
        #     lambda x: tf.image.resize_nearest_neighbor(*x),
        #     name="fpn_p5upsampled")([P5, P4_size])

        # P5_upsampled = tf.image.resize_nearest_neighbor(
        #     P5, P4_size[1:3], name="fpn_c4p4")
        P4 = KL.Add(name="fpn_p4add")(
            [P5_upsampled, KL.Conv2D(256, (1, 1), name='fpn_c4p4')(C4)])
        # set_trace()
        # P4 = KL.Add(name="fpn_p4add")([
        #     KL.UpSampling2D(size=(2, 2), name="fpn_p5upsampled")(P5),
        #     KL.Conv2D(256, (1, 1), name='fpn_c4p4')(C4)
        # ])
        P4_upsampled = UpSamplingTf(name="P4_upsampled")([P4, C3])
        P3 = KL.Add(name="fpn_p3add")(
            [P4_upsampled, KL.Conv2D(256, (1, 1), name='fpn_c3p3')(C3)])
        P3_upsampled = UpSamplingTf(name="P3_upsampled")([P3, C2])
        P2 = KL.Add(name="fpn_p2add")(
            [P3_upsampled, KL.Conv2D(256, (1, 1), name='fpn_c2p2')(C2)])
        # set_trace()
        ...
        #     config.RPN_ANCHOR_STRIDE)
        # self.anchors = utils.generate_pyramid_anchors(
        #     K.int_shape(input_image), config.RPN_ANCHOR_SCALES,
        #     config.RPN_ANCHOR_RATIOS, config.BACKBONE_STRIDES,
        #     config.RPN_ANCHOR_STRIDE)
        # set_trace()
        ...
        rpn_rois = ProposalLayer(
            ...
            config=config)([rpn_class, rpn_bbox, input_anchors, input_image])
        # set_trace()
        if mode == "training":
            ...
            if not config.USE_RPN_ROIS:
                ...
                target_rois = KL.Lambda(
                    lambda x: K.cast(x, tf.float32) / image_scale[:4])(
                        input_rois)
            else:
            ...
            mrcnn_mask = build_fpn_mask_graph(
                rois, mrcnn_feature_maps, input_image, config.MASK_POOL_SIZE,
                config.NUM_CLASSES)
            ...
            # Model
            inputs = [
                input_image, input_image_meta, input_anchors, input_rpn_match,
            ...
            _, input_image_shape, _, _ = parse_image_meta_graph(
                input_image_meta)

            # h, w = config.IMAGE_SHAPE[:2]
            h = input_image_shape[0][0]
            w = input_image_shape[0][1]
            # detection_boxes = detections[..., :4] / K.stack([h, w, h, w])
            # set_trace()
    ...
    def set_log_dir(self, model_path=None):
        ...
        self.checkpoint_path = self.checkpoint_path.replace("*epoch*",
                                                            "{epoch:04d}")
...
def compose_image_meta(image_id, image_shape, active_class_ids):
    ...
    meta = np.array([image_id] +  # size=1
                    ...
                    list([0, 0, image_shape[0], image_shape[1]]) +  # size=4
...
def parse_image_meta_graph(meta):
    ...
    image_id = meta[:, 0]
    image_shape = meta[:, 1:4]
    # window = K.stack([[0,0, image_shape[0][0], image_shape[0][1]]])
    window = meta[:, 4:8]
    active_class_ids = meta[:, 8:]
    return [image_id, image_shape, window, active_class_ids]
