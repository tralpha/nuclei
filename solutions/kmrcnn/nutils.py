...
def generate_pyramid_anchors(img_shape, scales, ratios, feature_strides,
                             anchor_stride):
...
def iou_metric(y_true_in, y_pred_in, print_table=False):
    ...
    assert y_true_in.shape[:2] == y_pred_in.shape[:2], "Check metric"
    # Is the below required? Are you sure?
    # labels = label(y_true_in > 0.5)
    # y_pred = label(y_pred_in > 0.5)
    height, width = y_true_in.shape[:2]
